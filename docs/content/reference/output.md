...
describes what users get. Pick a format with `-o`, or let mangadex choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mangadex <command> -o table   # aligned columns for reading
mangadex <command> -o jsonl   # one JSON object per line, for piping
mangadex <command> -o json    # a single JSON array
mangadex <command> -o csv     # spreadsheet friendly
mangadex <command> -o tsv     # tab-separated
mangadex <command> -o url     # just the URL column
mangadex <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
mangadex <command> --fields id,title,url
```
...
```bash
mangadex <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
mangadex <command>            # a table, because this is a terminal
mangadex <command> | wc -l    # JSONL, because this is a pipe
```
